# Units — Job Application Management Site

## Overview

The 15 user stories are grouped into 3 independently buildable units. Each unit is internally cohesive and loosely coupled to other units via well-defined interfaces.

| Unit | Name | Stories | Team Focus |
|------|------|---------|------------|
| 1 | Identity & Profile Management | US-1.1–1.6 | Authentication, user data, file storage |
| 2 | Job Search & Discovery | US-2.1–2.4, US-3.1–3.2 | Search engine, job data, browsing UX |
| 3 | Job Application & Tracking | US-4.1–4.3 | Application workflow, status tracking |
...
```
Unit 1 (Identity & Profile)
  ├──> Unit 2 (Job Search & Discovery)
  │      - Consumes: Authenticated user session (for bookmarks)
  │      - Interface: Auth token / user ID
  │
  └──> Unit 3 (Job Application & Tracking)
         - Consumes: User profile data, resume file
         - Interface: Profile API (name, email, phone), Resume API (file retrieval)

Unit 2 (Job Search & Discovery)
  └──> Unit 3 (Job Application & Tracking)
         - Consumes: Job posting details
         - Interface: Job API (job ID, title, company)
```

**Coupling contracts:**
- Unit 1 exposes: Authentication service, Profile API, Resume API
- Unit 2 exposes: Job API (job details by ID, job listing data)
- Unit 3 consumes from Unit 1 and Unit 2 but exposes no interfaces to them

---

## Unit 1 — Identity & Profile Management

**Description:** Handles all applicant identity concerns — registration, authentication, session management, profile data, and resume storage. This is the foundational unit with no dependencies on other units.

**Dependencies:** None (foundational unit)

**Interfaces Exposed:**
- Authentication service (login, session validation, logout)
- Profile API (read profile: name, email, phone, location, summary)
- Resume API (retrieve uploaded resume file)
...
**Description:** Handles all job browsing concerns — searching, filtering, sorting, pagination, viewing job details, and bookmarking. Provides a read-only view of job data for applicants.

**Dependencies:**
- Unit 1: Authenticated user session (required for Save/Bookmark feature only; search and viewing work for unauthenticated users)

**Interfaces Exposed:**
- Job API (retrieve job details by ID: title, company, location, type, level, salary, description, responsibilities, qualifications, posting date)
...
**Description:** Handles the application lifecycle — submitting applications, tracking their status, and withdrawing. This is the most dependent unit, consuming data from both Unit 1 and Unit 2.

**Dependencies:**
- Unit 1: Profile data (name, email, phone) for pre-filling; Resume file for attachment
- Unit 2: Job posting details (job ID, title, company) for the application form and application list

**Interfaces Exposed:** None (end-of-chain unit)
...
| Unit | Stories | Must Have | Should Have | Nice to Have |
|------|---------|-----------|-------------|--------------|
| Unit 1 — Identity & Profile | 6 | 5 | 1 | 0 |
| Unit 2 — Job Search & Discovery | 6 | 3 | 3 | 0 |
| Unit 3 — Job Application & Tracking | 3 | 2 | 0 | 1 |
| **Total** | **15** | **10** | **4** | **1** |
